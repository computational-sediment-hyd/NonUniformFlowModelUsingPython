...
class section(object):
    ...
    # def __init__(self, X, Y, N, distance, treeh, subsectBoundf, subsectBoundfd):
    ...
    def _setMixLength(self, H):
        ...
        for n in range(1, num-1):
            dep = np.max(np.array([H - ss[n].y[0], zero]))
            if dep > 0.0 :
                treeh = np.max(np.array([ss[n-1].treeh, ss[n].treeh]))
                if dep <= treeh:
                    subB[n].Swd = zero
                    if ss[n-1].treeh > ss[n].treeh :
                        subB[n].Swl = zero
                        subB[n].Swr = np.max(np.array([dep - ss[n].treeh, zero]))
                    else:
                        subB[n].Swl = np.max(np.array([dep - ss[n-1].treeh, zero]))
                        subB[n].Swr = zero
                else:
                    subB[n].Swd = np.max(np.array([dep-treeh, zero]))
                    if ss[n-1].treeh > ss[n].treeh :
                        subB[n].Swl = zero
                        subB[n].Swr = np.max(np.array([ss[n-1].treeh-ss[n].treeh, zero]))
                    else:
                        subB[n].Swl = np.max(np.array([-ss[n-1].treeh+ss[n].treeh, zero]))
                        subB[n].Swr = zero
            else:
                subB[n].Swd = zero
                subB[n].Swl = zero
